    ...
    def _make_combined_json(self, args):
        ...
        if isinstance(paths, list):
            ...
            for idx, p in enumerate(paths):
                ...
                print("previously image path",image_paths)
                img_path = image_paths[idx] if isinstance(image_paths, list) else image_paths
                print("img_path is here...",img_path)
                with open(p, 'r') as f:
                    data = json.load(f)
                    for item in data:
                        item['volume_path'] = img_path + os.sep + item['case_id']
                        print("volume path is being created",img_path + os.sep + item['case_id'])
                    combined.extend(data)
                    print("combined is here....",combined)
            json_file = combined
        else:
            ...
            with open(paths, 'r') as f:
                ...
                for item in data:
                    item['volume_path'] = img_path + os.sep + item['case_id']
            json_file = data
        print("josn file being created is here", json_file)
        return json_file
